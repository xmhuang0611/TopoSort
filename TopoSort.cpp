...
{
	FILE *fp = fopen("course_inf.txt", "rt");
	read readCourse[MAXCOURSE];
	if (!fp)
	{
		printf("errror!\n");
		exit(-1);
	...
	for (int i = 0; i < MAXCOURSE; i++)
	{
		...
		fscanf(fp, "%s%s%s", number, name, semester);//读入课程基本信息
		readCourse[i].number = number;
		readCourse[i].name = name;
		readCourse[i].semester = semester;
		for (int s = 0; s < MAXPREDE; s++)           //读入课程的先修课程信息
		{
			fscanf(fp, "%s", preDe[s]);
		...
		if (i == 0)
		{
			...
			{//读入先修课程信息
				...
				else
				{
					...
					newNode->inDegreeNumber = newNode->inDegreeNumber + 1; //入度加一
...
void TopoSort::disperseCourseToSemeter()
{
	...
	{//排有确定学期的课程
		if (topoSemester[i] != "0")
		{
			if (topoSemester[i] == "1")//课程对应学期为第一学期
			{
				semeter1[numberOfsemeter1] = topoCourseName[i];//将该课程加入到第一学期课程信息中
				numberOfsemeter1++;//已有第一学期课程数自增一
			}
